export function ProjectOverviewCard({ title, body, Icon, active = false }) {
    return (
        <article
            className={`project-overview-card w-[85vw] max-w-[24rem] shrink-0 snap-center border border-slate-200/10 bg-slate-950/38 p-4 shadow-[0_12px_40px_rgba(2,8,23,0.24)] backdrop-blur-[2px] sm:w-auto sm:max-w-none sm:shrink sm:snap-none sm:p-5${active ? " is-active" : ""}`}
        >
            ...
            <h2 className="mt-4 font-montserrat text-[1.2rem] font-semibold tracking-tight text-slate-100 sm:text-[1.45rem]">
                {title}
            </h2>
