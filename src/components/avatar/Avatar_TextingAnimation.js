...
export function AvatarTextingAnimation(props) {
    const { basePath } = useRouter()
    const { nodes, materials } = useGLTF(basePath + '/models/avatar_2.glb')

    const group = useRef()

    const { animations: textingAnimation } = useFBX(basePath + "/animations/Texting.fbx")

    textingAnimation[0].name = 'Texting'

    const { actions } = useAnimations(textingAnimation, group)
